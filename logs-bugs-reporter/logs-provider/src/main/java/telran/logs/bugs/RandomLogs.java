...
private void fillLogTypeArtifactMap(EnumMap<LogType, String> res, LogType lt) {
	...
	case AUTHENTICATION_EZCEPTION: 
		res.put(LogType.AUTHENTICATION_EZCEPTION, "authentication");
		break;
	...
	case DUBLICATED_KEY_EXCEPTION:
		res.put(LogType.DUBLICATED_KEY_EXCEPTION, getRandomClassName());
		break;
	case  NOT_FOUND_EXCEPTION:
		res.put(LogType. NOT_FOUND_EXCEPTION, getRandomClassName());
		break;
	case NO_EXEPTION:
		res.put(LogType. NO_EXEPTION, getRandomClassName());
		break;
...
private LogType getLogType() {
	...
	return chance <= exceptionProb ? getExceptionLog() : LogType.NO_EXEPTION;
}
...
private LogType getNonSecurityExceptionLog() {
	LogType nonSecExceptions[] = {
			LogType.BAD_REQUEST_EXCEPTION, LogType.DUBLICATED_KEY_EXCEPTION,
